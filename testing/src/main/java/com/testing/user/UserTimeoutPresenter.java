...
public class UserTimeoutPresenter {
  ...
  /**
   * Create {@link UserTimeoutPresenter} instance.
   *
   * @param listener - listener interface for interaction with views.
   * @param nameRepository - for getting user name.
   */
  public UserTimeoutPresenter(Listener listener, RxNameRepository nameRepository) {
  ...
  void getUserName() {
    Preconditions.checkState(
        !userNameDisposable.isPresent(), "Presenter has already loading user name.");
    userNameDisposable =
        Optional.of(
            nameRepository
                .getName()
